...
pub fn parse(tokens: &[String]) -> ParseNode {
    ...
    combine_arithmetic_expressions(&mut root);

    root
}

#[allow(clippy::collapsible_if)]
fn combine_arithmetic_expressions(node: &mut ParseNode) {
    // combine arithmetic expressions
    let mut continue_parsing = true;
    while continue_parsing {
        continue_parsing = false;
        for i in 0..node.children.len() - 1 {
            if is_arithmetic_operator(&node.children[i]) {
...
fn is_arithmetic_operator(node: &ParseNode) -> bool {
